...
export const QUESTION_SET_V9: QuestionSetItem[] = [
...
console.log(`Total questions in V9 set: ${QUESTION_SET_V9.length}`);
console.log(
  `Relevant: ${QUESTION_SET_V9.filter((q) => q.expectedCategory === 'relevant').length}`,
);
console.log(
  `Irrelevant: ${QUESTION_SET_V9.filter((q) => q.expectedCategory === 'irrelevant').length}`,
);
console.log(
  `Dangerous: ${QUESTION_SET_V9.filter((q) => q.expectedCategory === 'dangerous').length}`,
);

// bunx ts-node --require tsconfig-paths/register src/modules/evaluator/question-classification/test-sets/question-set-v9.constant.ts
