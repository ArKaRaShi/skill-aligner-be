 ...
 * Cost Analytics CLI
 *
 * Inspect query cost analytics from the command line.
 *
 * Usage:
 *   bunx ts-node --require tsconfig-paths/register src/modules/query-logging/cli/cost-analytics.cli.ts [OPTIONS]
 *
 * Options:
...
Query cost analytics and statistics from the database.
...
async function showSummary(): Promise<void> {
  ...
  try {
    const costAnalytics = appContext.get(QueryAnalyticsService);

    const avgCost = await costAnalytics.getAverageCost();
    const costStats = await costAnalytics.getCostBreakdownStats();
    const tokenStats = await costAnalytics.getTokenBreakdownStats();
...
async function showPerRunCosts(limit: number): Promise<void> {
  ...
  try {
    const costAnalytics = appContext.get(QueryAnalyticsService);
    const runs = await costAnalytics.getPerRunCosts(undefined, limit);
